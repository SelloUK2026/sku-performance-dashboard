...
## Tesco second-stage records

- The latest Tesco Catalogue upload is saved privately and is reused when an
  already-submitted nomination form is imported later.
- Under Saved Tesco events, use Import submitted nomination for a form sent
  outside this tool. The event name and dates are read from the form and can be
  corrected in the review window before saving.
- Forms generated by this tool contain an Internal Record tab. Its exact Tesco
  SKU takes priority, preserving alternate listings such as a SKU ending `_1`.
- For Tesco's standard form, the barcode is matched against the latest saved
  Tesco Catalogue. A unique match is accepted automatically. A missing or
  duplicate-barcode match must be confirmed by selecting an exact Tesco SKU.
- Saving an external form creates the same frozen overlap record as generating
  a form in the tool. Future overlapping events block only the same Tesco SKU;
  another Tesco listing for the same Wooper product remains available.
- Each saved event has a plain-text Discount end date in Mirakl field. Enter
  the internal Mirakl identifier exactly as staff need to see it and select
  Save. The field is deliberately not restricted to a calendar date.
...
- Added editable Discount end date in Mirakl text to each shared Tesco event.
- Added import and review of nomination forms submitted outside the tool.
- Persisted the latest Tesco Catalogue privately for barcode-to-Tesco-SKU
  matching, while requiring confirmation for missing or ambiguous matches.
- Kept the external-event workflow bilingual and included it in the in-app
  Tesco guide.
